...
package com.vramku.app.data.jpa;
...
public class SampleDataRestApplicationTests {
	...
	public void findByFirstNameAndLastName() throws Exception {
		...
		this.mvc.perform(
				get("/api/users/search/findByFirstNameAndLastNameAllIgnoringCase?firstname=Karthik&lastname=Ramkumar"))
	...
	public void findByContaining() throws Exception {
		...
		this.mvc.perform(
				get("/api/users/search/findByFirstNameContainingAndLastNameContainingAllIgnoringCase?firstname=Karthik&lastname=Ramkumar"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("_embedded.users", hasSize(1)));
	}
}
